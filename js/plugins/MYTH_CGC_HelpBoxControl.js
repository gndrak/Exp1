...
* @target MZ
* @author Swarnava Banerjee (Neel)
* @plugindesc v1.2.0 バトル中のヘルプボックスの設定を変更できるようにしました。CGCと互換性があります。
* @url https://mythatelier.itch.io/card-game-combat
*
* @param Help Box Settings
*
...
* @parent Help Box Settings
* @text テキストを折り返す？
* @desc ヘルプボックス内のテキストは、ウィンドウ幅に基づいて折り返されます。
* @type boolean
* @default true
*
* @param helpBoxScreenBuffer
* @parent Help Box Settings
* @text スクリーン境界のバッファ
* @desc 表示領域の端からどの程度離すかのバッファ。0に設定すると、表示領域サイズに固定しない。
* @type number
* @default 32
*
* @param helpBoxCardBuffer
* @parent Help Box Settings
* @text カードオフセットバッファ
* @desc カードの端からどれだけ離すかのバッファ。負の場合、選択したカードと重なる。
* @type number
* @default 4
*
* @param helpBoxFontFace
* @parent Help Box Settings
* @text フォント名
* @descヘルプボックスのフォント名（フォントフォルダーと同じ名前であること）
* @type string
* @default GameFont
*
* @param helpBoxFontSize
...
* @parent Follow Conditions
* @text セレクトカードに従う？
* @desc ヘルプボックスが現在選択されているカードにオフセットして従うかどうかを決定する。
* @type boolean
* @default true
*
* @param helpBoxAlign
* @parent Follow Conditions
* @text カード整列
* @desc ヘルプボックスのX/Yを選択されたカードの位置に対して自動的に設定します。”Don't Use "の場合、デフォルトはアンカーX/Yになります。
* @type select
* @option Don't Use
* @option Top
* @option Left
...
* @parent Follow Conditions
* @text フォロー時にXをロックする？
* @desc Trueの場合、ヘルプボックスのXを固定し、インデックスによって緩衝しないようにします。
* @type boolean
* @default false
*
* @param followHBLockY
* @parent Follow Conditions
* @text フォロー時にYをロックする？
* @desc Trueの場合、ヘルプボックスのYを固定し、インデックスによって緩衝しないようにします。
* @type boolean
* @default false
*
* @param useSmartAlign
* @parent Follow Conditions
* @text スマート整列を使用する？
* @desc 使用する場合、表示領域の境界に基づいて整列させる。(例：ヘルプボックスが端からはみ出る場合、左寄せ→右寄せ)
* @type boolean
* @default true
*
* @param anchorFallback
* @parent Follow Conditions
* @text アンカーフォールバックを使う？
* @desc Trueの場合、カードハンドの外側を検査する際、ヘルプボックスはアンカーX Yにフォールバックします。
* @type boolean
* @default false
*
* @param State Popup Integration
...
* @parent State Popup Integration
* @text ウィンドウはホスト状態？
* @desc MythStateWindow を使っている場合は、ヘルプボックスの下にあるウィンドウをホストします。
* @type boolean
* @default false
*
* @param allowDynamicOffset
* @parent State Popup Integration 
* @text ダイナミックオフセットを許可する？
* @desc ONの場合、ヘルプボックスを現在のステートウィンドウの数に相対的にオフセットします。オフの場合、ヘルプボックスはステートウィンドウの最大数を基準にオフセットされます。
* @type boolean
* @default true
*
* @param alignOverride
* @parent State Popup Integration
* @text整列状態の上書き
* @desc この機能は、ステートウィンドウが1つの構成のみで表示されるようにしたい場合(カードアラインを無視)に使用します。
* @type select
* @option Don't Use
* @option Top
* @option Left
 ...
 * ============================================================================
* 
* このプラグインは、ヘルプウィンドウを現在選択されている手札のカードに追従する、
* 移動するツールTipsに変更することができます。
* ヘルプウィンドウの寸法は、ゲームのニーズに合わせて変更可能です。
* ヘルプウインドウが移動していないとき、または他の機能によってオフセット
* されていないとき、画面上のアンカーポイントを設定することができます。
* 現在のカードに対するヘルプウィンドウの配置を変更することもできます。
* (ウィンドウの配置を変更することができます。）
*
* このプラグインはIsiahStateWindowプラグインによって作成された
* ステートウィンドウをサポートします。ステートウィンドウの配置を変更して、
* カードの別の場所に表示したり、ゲームのUIに適した新しい構成にしたりできます。
* これらのUIエレメントをフルに活用するには、プロジェクトの解像度を
* 1280 x 720 px以上にすることをお勧めします。
* （そうしないと、画面が少し窮屈に感じるかもしれません）
* 
*
* ヘルプウィンドウやステートウィンドウがスクリーンの端に近すぎる場合、
* プラグインパラメータを介して画面境界バッファとスマート配置を有効にして、
* 画面内にすべてのウィンドウを表示する最適な方法をプラグインに判断させることが
* できます
*
* MythCardGameCombat と IsiahStateWindow プラグインで動作するようにしました。
* これらのプラグインをこのプラグインの上に置くことで、本プラグインの機能が
* 動作するようになります。
* ============================================================================
* バージョン履歴
* ============================================================================
*
* v1.2.0 - 文字列の折り返しが大幅に改善され、エスケープ文字で途切れることがなくなりました。
*		 - CoreEngineの変更により、ヘルプウィンドウの表示プラグインパラメータが動作しなかった問題を修正
*
* v1.1.1 - Party UI A、アンカー、カード整列の競合を修正しました。
*        - 説明が途中で切れてしまわないよう、基本的な文字列の折り返しを追加しました。
*        - ヘルプウィンドウがPartyコマンドに戻った際に閉じないバグを修正しました。
*        - いくつかのパラメータをよりわかりやすいカテゴリーに統合しました。
*
* v1.1.0 - いくつかの機能をコアプラグインに統合しました。
 *        - プラグイン名を変更。
 * 
* v1.0.3  - MZ の機能と複数の解像度での互換性をテストしました。
*         - ヘルプボックスの位置揃えプラグインパラメータを追加 (上、左、右、下)
*         -  スマートアラインオプションを追加しました。
*         - ウィンドウとカードの重なりを制限するために、カードオフセットバッファを追加。
*         - ステートウィンドウの配置を変更するための IsiahStateWindows の統合を追加しました。
*         - ステートウィンドウの追加設定のためのプラグインパラメータを追加。
*
* v1.0.2  - カードがコストとして破棄または削除された場合でもヘルプボックスは残ります。
*         - マウスやゲームパッドで選択した後、ヘルプボックスが再配置されるようになりました。
*         - ヘルプボックスが画面の範囲内に収まるように viewportClamp 機能を追加しました。
*         - ヘルプ情報ボックスにデッキとディスカードの説明を追加しました。
*         - プラグインパラメータでヘルプボックスウィンドウのスキンを変更できるようになりました。
*         - 検査がカードハンドの外に出た場合、AnchorFallbackを追加しました。
*
* v1.0.1  - コマンド選択オーバーライドの追加
*
* v1.0.0 - 基本機能完成。手札のインデックスを元にヘルプボックスを移動できます。
*/
