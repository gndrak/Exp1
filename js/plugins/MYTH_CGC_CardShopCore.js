 ...
 *         
 * Only one shop preset can be made per event, but until the shop is reset
 ...
 * 
 * Price formulas have several 2 variables you can use:
 ...
 * @target MZ
 * @author Isiah Brighton
 * @plugindesc v1.2.0 カードの売買を可能にするメニューを追加する、MYTH_CGC_CoreEngineの拡張プラグインです。
 * @url https://mythatelier.itch.io/card-shop-cgc-expansion-plugin
 *
* @command openCardShop
 * @text カードショップを開く
 ...
 * @type number
 * @default 10
 * @desc ショップのカードのデフォルト価格。メモタグで上書き可能。
 * 
 * @param buyPriceFormula
 * @text 購入価格の計算式
 * @type text
 * @default adjustedCardPrice
 * @desc カードの購入価格を決定する計算式。詳細はヘルプを参照。 *
 * @param sellPriceFormula
 * @text Sell Price Formula
 * @type text
 * @default sellPrice || Math.floor(buyPrice / 2)
 * @desc カードの売却価格を決定する式。詳細はヘルプを参照。
 *
 * @param removePriceFormula
 * @text Remove Price Formula
 * @type text
 * @default (removePrice || buyPrice) + 10 * removed
 * @desc カードの削除価格を決定する式。詳細はヘルプを参照。
 *
 * @param freeText
 * @text 無料カードテキスト
 * @default 0 \c[16]Gold
 * @desc 0ゴールドのカードに表示するテキスト。
 * 
 * 
 * @param shopScene
 * @text カードショップシーンパラメータ
 * 
 *   
 *   @param previewCoords
 *   @parent shopScene
 *   @text プレビュー座標
 *   @type struct<Coordinate>
 *   @desc カードショップのシーンにおけるプレビューカードの座標。
 *   @default {"x":"140","y":"450"}
 *
 *   @param previewScale
 *   @parent shopScene
 *   @text プレビューカードサイズ
 *   @desc カードショップのシーンにおけるプレビューカードのサイズ。
 *   @type number
 *   @decimals 2
 *   @default 1.0
 *   
 ...
 *    @type number
 *    @default 2
 *    @desc 垂直スクロールするまでの待ち時間を計算するために使用される数値。
*
 *    @param cardDisplay
 *    @text カード配置設定
 *    @parent section2
 *    @type struct<CardDisplay>
 *    @default {"maxCols":"6","cardScale":"1","selectedCardScale":"1.1","startPadding":"{\"x\":\"50\",\"y\":\"80\"}","cardSpacing":"{\"x\":\"30\",\"y\":\"40\"}","amountCoords":"{\"x\":\"0\",\"y\":\"0\"}","amountJustify":"right"}
 *    @desc ライブラリウィンドウ内のカードの配置を決定する設定。
 *   
 *    @param costWindowWidth
 *    @parent section2
 *    @text コストウィンドウ幅
 *    @type number
 *    @default 200
 *    @desc購入画面の価格のテキストが切れてしまう場合は、これを大きくすることができます。
 *    
 *    @param costWindowFontSize
 *    @parent section2
 *    @text コストフォントサイズ
 *    @type number
 *    @default 18
 *    @desc 購入ウィンドウのカードのコストのフォントサイズ。
 * 
 *   @param cardListWidth
 *   @parent shopScene
 *   @text カード一覧ウィンドウの幅
 *   @desc カード一覧ウィンドウのピクセル単位の幅
 *   @type number
 *   @default 576
 * 
 *   @param confirmWidth
 ...
 *   @min 0
 *   @default 300
 *   @desc ゴールドウィンドウの幅。カードプレビューの表示スペースを決定します。
 *   
 *   @param hideActorWindow
 *   @text アクターウィンドウを隠しますか?
 *   @parent shopScene
 *   @type boolean
 *   @default false
 *   @desc ONに設定すると、パーティメンバー複数でカードを購入する際にのみ、アクターウィンドウが表示されます。
 *   
 *   @param actorWindowWidth
 *   @text アクターウィンドウ幅
 *   @parent hideActorWindow
 ...
 *   @min 0
 *   @default 300
 *   @descアクターウィンドウの幅。「アクターウィンドウを隠す」がオフの場合のみ使用されます。
 *   
 *  @param shortActorWindow
 *  @text アクターウィンドウを短くする?
 *  @parent hideActorWindow
 *  @type boolean
 *  @default true
 *  @desc ONに設定すると、アクターウィンドウに表示される情報が少なくなります。
 *     
 *  @param altUI
 *  @parent shopScene
 *  @text ヘルプウィンドウを移動
 *  @type boolean
 *  @default false
 *  @desc ONに設定すると、ヘルプウィンドウがカードプレビューの下に表示され、カードリストの表示領域が大きくなります。
 *  
 *  @param altUIHelpRows
 *  @parent shopScene
 *  @text ヘルプウィンドウの行数
 *  @type number
 *  @default 2
 *  @desc ヘルプウィンドウに表示できるテキストの行数を設定します。
 *    
 *   @param scrollSettings
 *   @text スクロールバーの設定
 *   @parent shopScene
 ...
 *     @dir img/system
 *     @require 1
 *     @desc スクロールバーに使用されるスプライト。その高さの3分の1が、バーのサイズを変更するために拡大縮小されます。
 *
*     @param scrollBack
 *     @parent scrollSettings
 *     @text バックスプライト
 ...
 *     @type number
 *     @default 16
 *     @desc スクロールバーが表示されるスクロールウィンドウの右側のピクセル数。
 *
 *     @param scrollBarYPinch
 *     @parent scrollSettings
 *     @text Yピンチ
 ...
 * @type struct<CardPack>[]
 * @default []
 * @desc カードショップに追加できるカードパックの一覧です。
 * 
 * @param giveWholePackToActor
 * @text アクターに全パックを渡す？
 * @type boolean
 * @default false
 * @desc If ONの場合、カードパックを購入すると、選択したアクターにすべてのカードが渡されます。OFFの場合、各カードは異なる俳優に渡すことができます。
 * 
 * @help
 *
 * 
 ...
 * ============================================================================
 * 
 * このプラグインはカードショップシーンを追加します。
 * 
 * カードショップは以下のプロセスで開くことができます：
 * 
 *     1. イベントまたはコモンイベントで注釈を使って、ショップの内容や、プレイヤーがカードを買ったり、売ったり、取り除いたりできるかどうかを指定するメモタグを追加します。
*     2.同じイベントか共通イベントで、this.cardShopProcessing()というスクリプトを呼び出します
 *         
 *         
 * ショップのプリセットは1つのイベントにつき1つしか作成できませんが、ショップがリセットされるまで、その在庫はセッション間で引き継がれます。つまり、プレイヤーがそのショップの在庫をすべて購入した場合、プレイヤーがショップに戻っても、在庫は空のままです。
* 
 * ============================================================================
 * 購入、売却、撤去
 * ============================================================================
 * 
 * 各カードショップでは、プレイヤーがカードを購入、売却、およびカードを取り除くことができるかどうかを指定することができます。
 * 
 * 購入 - イベントコメントタグで指定したインベントリに基づき、アクターのライブラリーにカードを追加することができます。
 * 売却 - プレイヤーは、カードの購入コストの半分と引き換えに、自分の ライブラリーからカードを取り除くことができます。 
 * 除去- プレイヤーは自分のライブラリーからカードを取り除くことができますが、お金を受け取る代わりに、Remove Priceプラグインパラメーターのアルゴリズムに基づいてお金を使わなければなりません。
 * 
 * 
 * ============================================================================
 * スキルのメモタグ
 ...
 * <Card Buy Price: X>
 * 
 * これにより、そのスキルのカードの価格が X に設定されます。
* 価格をゲーム変数で置き換えるには、x[x] を使用します。
 * 
 * <Card Sell Price: X>
 * 
 * これにより、そのスキルのカードの売却価格が X に設定されます。
 * 価格をゲーム変数に置き換えるには、v[x] を使用します。
 * 
 * <Card Remove Price: X>
 * 
 * これにより、そのスキルのカードの除去価格がXに設定されます。
 * 価格をゲーム変数に置き換えるには、x[x]を使用します。
 * 
 * <Shop Image: fileName>
 * 
 *これにより、カードショップで指定した画像を使用して、そのスキルのカードが表示されるようになります。画像はimg/CGC/shopに配置してください。
* 
 *  * ============================================================================
 * イベント注釈タグ
 * ============================================================================
 *
 * カードショップのシーンを開く前に、ショップの内容を指定する必要があります。イベントページ内の1つまたは複数の注釈で指定できます。
 * 
 *     <card shop>
 *     info
 *     info
 *     </card shop>
 * 
 * イベントページでは、複数の注釈にまたがって情報を配置することができます。
 * 
 *  開始タグと終了タグの間に、以下の構文を使用してカードショップに情報を入力することができます：
 * 
 *     <card shop>
 *     3x Skill Y
 *     5x Skill Z
 *     </card shop>
 * 
 * YとZは入れたいカードのスキルIDです。[N]xはショップに入れたいカードの枚数を表します。このフォーマットも使えます：
 *
 *     <card shop>
 *     5x skillName
 *     3x skillName
 ...
 *     </card shop>
 *
 * ここで、skillName は含めたいカードの名前です。同じ名前のカードが複数ある場合、データベース内の最初のカードを取得します。
 *
 * デフォルトと異なる価格のカードが欲しい場合は、これを使う：
 * 
 *     Skill Y Price Z
 * 
 * YはスキルID、Zはカスタム価格です。
 ...
 *     2x skillName Price Z
 *     
 * さらに、カードパックプリセットパラメータで設定されたカードパックを追加できます。
 * 
 * 
 *     Pack Preset Y
 *     3x Pack Preset Y
 ...
 *     Command: buy, sell, remove, cancel
 *     
  * どのコマンドがシーンに存在するかを決定します。1つしか存在しない場合、コマンドウィンドウは見えなくなり、カード選択ウィンドウに多くのスペースが与えられます。
* 
 * 
 *     Buy price priceFormula
 *     
 * これにより、そのショップの計算式が設定され、すべてのカードのコストが変更されます。
 * 
 *     Sell price priceFormula
 *     Remove price priceFormula
 *     
 * これらはカードの売却や除去の際に、カードの価値を決定するための公式を上書きします。デフォルトのSell式はカードの通常価格の半分で、デフォルトのRemove式はRemove Price Formulaプラグインパラメーターによって設定されます。
 *
 * 
 * 購入価格の計算式については、このヘルプの購入価格の価格計算式を参照してください。 * 
 * 
 *     Buy Limit X
 *     Sell Limit X
 *     Remove Limit X
 *     
 * これらは、購入/売却/削除のオプションが無効になる前に、それぞれ購入、販売、
 * または削除できるカードの数を決定します。
 * 
 * 
 * 最後に、このプラグインには、以下のようにこれらのコメント内の値をランダム化する
 * ツールがあります。
 * 
 *    1x Skill [1-4]
 *    
 * 数字を[最小値-最大値]に置き換えると、最小値から最大値(両端を含む)までの範囲で
 * ランダムな数字が決定されます。そのため、上記の例ではスキル1、スキル2、スキル3、
 * またはスキル4が表示される可能性があります。
 * 
 * また、このようにノートタグの冒頭に置くこともできます。
 * 
 *    [1-3]x Skill 4
 * 
 * これにより、ショップにスキル4のカードが1~3枚入荷します。
 * 
 * さらに、以下のものも使用できます。
 * 
 *    1x Skill [5, 7, 9]
 * 
 * 数字を[x, y, z, etc]に置き換えると、店舗は括弧内の数字のリストから選択することに
 * なります。したがって、上記の例ではスキル5、スキル7、またはスキル9が表示される
 * 可能性があります。
 * 
 * これはコメント内のどこでも機能し、ショップの在庫を決定します。
 * つまり、次のようなことができます。
 * 
 * <card shop>
 * [3-4]x Skill [7, 8, 9, 11, 13, 15]
 ...
 * </card shop>
 * 
 * これらのランダム化の方法では、整数値のみが生成されることにご注意ください。
 * また、スクリプトコールやプラグインコマンドによってショップの在庫がリセット
 * された場合でも、ランダムに生成された数値はその値を維持します。
 * 
 *  * ============================================================================
 * スクリプトの呼び出し
 * ============================================================================
 ...
 * this.cardShopProcessing();
 * 
 * 上記のイベント注釈タグによってカードショップが設定されている限り、カードショップシーンが開き、この関数を呼び出した同じイベントに属するショップに移動します。
 * 
 * this.resetCardShopInventory();
 * this.resetCardShopInventory(eventId);
 * 
 * これは、指定されたイベントIDのカード・ショップをリセットします（IDが指定されていない場合は、現在のイベントを使用します）。
 * これは、元の在庫に再入荷し、削除コストがリセットされることを意味します。
 * 
 *
 * ============================================================================
 * プラグインコマンド
 ...
 * OpenCardShop
 *
 * this.cardShopProcessing()と同じように、カードショップのシーンをイベントで開くための関数です。
 *
 * ResetCardShop
 * ResetCardShop [EventID]
 *
 * this.resetCardShopInventory() と同じように、ショップの元の在庫を補充し、Remove コストをリセットします。EventID]が指定されていない場合、デフォルトで現在のイベントのショップをリセットします。
*
*
============================================================================
 * Price Formulas
 * ============================================================================
 * 
 * 価格計算式はダメージ計算式と同じように機能しますが、カードの購入や除去にかかるコストや、売却時にプレイヤーに与える金額を決定します。
 * 
 * 価格計算式には2つの変数があります：
 *     buyPrice -  スキルのメモタグを通して設定されたカードの元の販売価格。
 *     sellPrice - スキルのメモタグを通して設定されたカードの元の売却価格。.
 *     removePrice ? スキルのメモタグを通して設定されたカードの元の削除価格。
 *     
 *     bought - そのショップで購入したカードの合計
 *     sold - その店で売却したカードの合計
 *     removed - その店で削除したカードの合計
 *     
 *     adjustedCardPrice - Buy 式専用の変数。
 *     このショップ専用のカードの価格です。
 *     
 * そこで、次のような例を使います：
 * 
 *     Remove price cardPrice + 10 * removed
 *     
 * カードの購入コストが40の時、プレイヤーがこのショップでカードを
 * 削除しなかった場合、削除コストは40となり、カードを削除するたびに
 * 価格が10ずつ増加します。
 * 
 * 例 2:
 * 
 *     Buy price adjustedCardPrice * 2
 * 
 * これにより、すべてのカードの価格が 2 倍になるショップが作成され、
 * さらに一部のカードには 2 倍のカスタム価格が設定されます。
 * 
 * 例 3:
 *
 *    Sell price Math.floor(cardPrice * 0.3)
 *
 * これにより、販売されたすべてのカードの価値の約 30% がゴールドで
 * 戻ってくるショップが作成されます。これは調整の影響を受けません。
 *
 * ============================================================================
 * Version History
 * ============================================================================
 * 
 * v1.2.0 - CoreEngine v1.6.2との互換性のために更新されました。
 *          ライブラリのオーバーホールの成果として、カードショップシーンの
 *          完全なオーバーホール。
 *          多くのUI改善。より多くのUIパラメータを追加。
 *          バックアウトしてショップに再入場した後、購入の計算式がデフォルトに
 *          リセットされるバグを修正。
 *          ショップのインスタンス間で、買った変数と売った変数がリセットされる
 *          バグを修正。
 *          デフォルトの売却計算式が機能していなかったのを修正。
 *          カードが空いているときに表示されるものを変更するパラメーターを追加。
 *          カードパックをショップに追加。
 *          カードメモタグにカスタム販売と削除価格を追加。
 *          カードをショップに表示する際に、カスタムイメージのメモタグを追加。
 *          <card shop>の設定タグに、Buy Limit、Sell Limit、Remove Limitの
 *          オプションを追加。
 *          <card shop>設定タグにランダム化オプションを追加。
 *           YEP_X_MoreCurrencies との互換性を追加。
 * 
 * 
 * v1.1.3 - スクロールバーの計算エラーを修正。
 *          コストテキストのYオフセット・パラメーターを追加し、パラメーターを少し整理しました。
 *          画面外をスクロールしていたカードが、スクロールし直すと届かなくなるのを修正。
 *          ヘルプを少し書き直しました。
 *          プラグインマネージャで、IsiahCardGameCombatがその上にない場合のエラー処理を追加。
* 
 * v1.1.2 - 1人のパーティーメンバーが購入を取り消せなかったエラーを修正。
 *
 * v1.1.1 - デフォルトの売買価格計算式にプラグインパラメータを追加しました。
 *        - カードショップのオープンとリセットのプラグインコマンドを追加
 *
 * v1.1.0 - MVのタッチ入力のバグを修正しました。
 *          カードがある他のシーンをロードする前に、初めてカードショップを開いた場合、プレビューカードの間違った場所にあるコストテキストを修正しました。
 *          特定の方法でコモンイベントを呼び出すとクラッシュするのを修正。
 *          ユーザーがライブラリーから隠されているカードを供給した場合の「ゴーストカード」を修正。
 *           デッキエディターを使用する際に、特別なフィードバックを追加しました。全てのカードが使用されている時にカードを売却/削除しようとした場合、 ヘルプウィンドウにその旨が表示されるようになりました。
 *          コマンドウィンドウに4つのアイテムを表示できるようになりました。
 *          ダミーウインドウが購入ウインドウと同じ大きさになりました。
 *          YEP_ShopMenuCoreとの互換性を追加しました。
 *          カスタム価格が数式を受け付け、cardPrice変数を使用できるようになりました。ゲーム変数を組み合わせることもできます。
 *           スクロールバーの追加。
 *          ヘルプウィンドウを縮小し、カードリストウィンドウを拡大する代替 UI デザインを導入。
 *          カードのカスタム価格を指定できるタグを追加しました。
 *          タグを追加し、購入、売却、削除（デフォルト値を上書き）のためのカスタム価格式を指定できるようにしました。
 *          
* 
 * v1.0.0 -プラグインリリース
 *
 ...
 * @text 説明
 * @default Buy a pack of 5 cards
 * @desc このカードパックをハイライトしたときにヘルプウィンドウに表示されるテキスト。
 *
 * @param amount
 * @text パック内のカード枚数
 * @type number
 * @min 1
 * @default 5
 * @desc このパックに含まれるカードの枚数。
 * 
 * @param cardPool
 * @text カードプール
 * @type struct<PoolCard>[]
 * @default []
 * @desc このパックのどのコピーにも入っている可能性のあるカード。
 *
 * @param price
 * @text 価格
 * @type number
 ...
 * @text カバーイメージ
 * @default Default
 * @desc img/CGC/shopにある、このパックの外観を作る画像。
 * 
 * @param note
 * @text メモタグ
 * @type note
 * @desc YEP_X_MoreCurrenciesのように、ここにメモタグを入れる必要がある場合。
 * @default ""
 *
 */
 ...
 * @min 1
 * @default 100
 * @desc プールから選ばれるカードの重さ。ウェイトが高いほど出現する可能性が高い。
 * 
 */
