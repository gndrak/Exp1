 ...
 * @target MZ
 * @author Isiah Brighton
 * @plugindesc v1.0.0 MYTH_CGC_CoreEngineの拡張で、プレイヤーが過去に所有していたすべてのカードを表示する新しいメニューが追加されました。
 * @url https://mythatelier.itch.io/card-game-combat
 * 
 * @command OpenCardCollection
 * @text カードコレクションを開く
 ...
 * @type boolean
 * @default false
 * @desc OFFに設定すると、カードを水平ではなく垂直にレイアウトする。
 *
 * 
 * @param numVisibleCols
 * @text 最大表示列数
 * @type number
 * @default 6
 * @desc 水平方向（Horizontal Collectionの場合は垂直方向）にスクロールするまでの待ち時間を計算するために使用する数値。
 *
 * @param numVisibleRows
 * @text 最大表示行数
 * @type number
 * @default 2
 * @desc 垂直方向（Horizontal Collectionの場合は水平方向）にスクロールするまでの待ち時間を計算するために使用する数値。
 *
 * @param cardDisplay
 * @text カード配置の設定
 * @type struct<CardDisplay>
 * @default {"maxCols":"6","cardScale":"1","selectedCardScale":"1.1","startPadding":"{\"x\":\"50\",\"y\":\"80\"}","cardSpacing":"{\"x\":\"30\",\"y\":\"40\"}"}
 * @desc コレクションウィンドウ内でのカードの配置を決定する設定。
 * 
 * 
 * @param missingCardBitmap
 * @text 欠落カード画像
 * @desc img/CGC/basesで、欠落カードに使用する画像。デフォルトのシェーディング効果を使用する場合は空白のままにしてください。
 *
 * @param missingCardDesc
 * @text 欠落カードの説明
 * @desc 欠落カードのヘルプウィンドウに表示される説明です。スキルの実際の説明は空白のままにしてください。
 * @default ???
 * 
 * @param showHelpWindow
 * @text ヘルプウィンドウを表示しますか？
 * @type boolean
 * @default true
 * @desc オフの場合、ヘルプウィンドウは表示されず、カードの縦方向のスペースが節約されます。
 *
 *
 * @param backgroundImage
 * @text 背景画像
 ...
 * ============================================================================
 *
 * このプラグインはメニューにCard Collectionオプションを追加します。
 * 
 * 
 * アクターがスキルを学んだり、装備や状態によってカードを獲得した場合、
 * そのカードがそのアクターから削除された後でも、そのカードはコレクションに表示されます。
 * 
 * 
 * さらに、コレクションから欠落しているカードは灰色で表示されるか、
 * さもなければ欠落しているカード画像パラメータを使用して表示されます。
 * 
 * カードコレクションにスキルが表示されないようにするには、以下を使用します。
 * Skill Notetag:
 *    <hide from collection>
 * 
 */
 ...
 * @type number
 * @default 6
 * @desc カードの最大列数。Horizontal Collectionを使用する場合は、代わりに最大行数として扱います。
 *
 * @param cardScale
 * @text カードスケール
 * @type number
 ...
 * @text スターティングパディング
 * @type struct<Coordinate>
 * @desc (不可視の)ウィンドウの左端/上端とカードの間のパディング。
 * @default {"x":"50","y":"80"}
 *
 * @param cardSpacing
 * @text カード間隔
