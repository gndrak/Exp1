 ...
 * @target MZ
 * @author Isiah Brighton
 * @plugindesc v1.2.0 カードテキストに書式オプションを追加
 * @url https://mythatelier.itch.io/card-game-combat
 *
* @param fontExtension
 * @text フォントファイル拡張子
 ...
 * @option .woff
 * @default .ttf
 * @desc このプラグインがフォントフォルダで探すべきファイルタイプ。
 * 
 * @param iconSize
 * @type number
 * @default 16
 * @text アイコンサイズ
 * @desc カード上でアイコンを縮小する必要があるかもしれません。これはアイコンのサイズです。タイプ アイコンはCardTypesのパラメータを使用します。
 *
 * @param Card Name
 * @type boolean
 * @default true
 * @desc これをOFFに設定すると、カードに名前が描画されなくなります。以下の内容は無視して結構です。
 * 
  *   @param nameFontSize
 *   @text フォントサイズ
 *   @parent Card Name
 ...
 *   @type text
 *   @default GameFont
 *   @desc カード名に使用するフォントの名前。デフォルトは "GameFont"。
 *   
 *   @param nameTextStyle
 *   @text テキストスタイル
 *   @parent Card Name
 ...
 *   @min -100
 *   @default 4
 *   @desc カード名のテキストスタイルの値。詳細はヘルプを参照。
 *   
 *   @param nameWidth
 *   @text 最大テキスト幅
 *   @parent Card Name
 ...
 *   @default 0
 *   @min -1
 *   @desc テキスト行が折り返されるまでのピクセル幅。0 で自動的に設定される。-1 で折り返しを無効にする。
 *   
 *   @param nameSpacing
 *   @text 行間
 *   @parent Card Name
 *   @type number
 *   @default 0
 *   @desc 複数行のテキスト間のスペースのピクセル数（該当する場合）。0 を指定すると自動的に設定されます。
 *
 *   @param nameAlign
 *   @parent Card Name
 *   @text テキストの配置
 ...
 *   @option right
 *   @default left
 *   @desc テキストが与えられたスペース内で左揃え、右揃え、または中央揃えになるかどうか。
 * 
 * @param Card Description
 * @type boolean
 * @default true
 * @desc これをOFFに設定すると、カード上に説明が描画されなくなります。以下の内容は無視して結構です。
 *
 * 
 * 
*   @param descFontSize
 ...
 *   @type text
 *   @default GameFont
 *   @desc カードの説明に使用するフォントの名前。デフォルトは "GameFont"。
 *   
 *   @param descTextStyle
 *   @text テキストスタイル
 *   @parent Card Description
 ...
 *   @option None
 *   @default Default
 *   @desc カードの説明のテキストスタイル。詳細はヘルプを参照。
 *   
 *   @param descTextStyleValue
 *   @text テキストスタイル値
 *   @parent Card Description
 ...
 *   @min -100
 *   @default 4
 *   @desc カードの説明のテキストスタイルの値。詳細はヘルプを参照。
 * 
 *   @param descWidth
 *   @text 最大テキスト幅
 *   @parent Card Description
 ...
 * @type boolean
 * @default true
 * @desc これをOFFに設定すると、カードに費用が請求されません。以下は無効となります。
 *
 * 
*   @param costFontSize
 *   @text フォントサイズ
 ...
 *   @type text
 *   @default GameFont
 *   @desc カードのコストに使用するフォントの名前。デフォルトは "GameFont"。
 *   
 *   @param costTextStyle
 *   @text テキストスタイル
 *   @parent Card Cost
 ...
 *   @option None
 *   @default Default
 *   @desc カードのコストのテキストスタイル。詳しくはヘルプを参照してください。
 *   
 *   @param costTextStyleValue
 *   @text テキストスタイル値
 *   @parent Card Cost
 ...
 *   @min -100
 *   @default 4
 *   @desc カードのコストのテキストスタイルの値。詳しくはヘルプを参照してください。
 *   
 *   @param costAlign
 *   @parent Card Card
 *   @text テキスト配置
 ...
 *   @option right
 *   @default right
 *   @descテキストが与えられたスペース内で左揃え、右揃え、または中央揃えになるかどうか。
 * 
 * @param Card Type
 * @type boolean
 * @default true
 * @descこれをOFFに設定すると、カード上にタイプが描画されなくなります。以下の内容は無視して結構です。
 *
 * 
 *   @param typeFontSize
 *   @text フォントサイズ
 ...
 *   @type text
 *   @default GameFont
 *   @desc カードのタイプに使用するフォントの名前。デフォルトは "GameFont"。
 *   
 *   @param typeTextStyle
 *   @text テキストスタイル
 *   @parent Card Type
 ...
 *   @option None
 *   @default Default
 *   @desc カードタイプのテキストスタイル。詳細はヘルプを参照。
 *   
 *   @param typeTextStyleValue
 *   @text テキストスタイル値
 *   @parent Card Type
 ...
 *   @min -100
 *   @default 4
 *   @desc カードのタイプのテキストスタイルの値。詳細はヘルプを参照。
 *   
 *   @param typeAlign
 *   @parent Card Type
 *   @text テキスト配置
 ...
 * @type boolean
 * @default false
 * @desc コストや説明文の変数計算など、ゲーム中にカードのテキストが変更された場合、これをtrueに設定します。
 * 
 * 
 * @param linebreak2
 * @text --------------------------
 ...
 *
 * @param SFont Compatibility
 * @desc SFontを使用している場合、Refresh Card Text以外の上記のパラメータは無視し、代わりにこれらを使用する必要があります。
 * 
 *   @param cardNameSFont
 *   @parent SFont Compatibility
 *   @text カード名 SFont
 ...
 *   @default -1
 *   @min -1
 *   @desc カードの説明に使用する SFont のインデックス。
 *   
 *   @param cardCostSFont
 *   @parent SFont Compatibility
 *   @textカードコスト SFont
 ...
 *   @default -1
 *   @min -1
 *   @desc カードコストに使用する SFont のインデックス。
 *   
 *   @param cardTypeSFont
 *   @parent SFont Compatibility
 *   @text カードタイプ SFont
 ...
 *   @default -1
 *   @min -1
 *   @desc カードタイプに使用する SFont のインデックス。
 *   
 *   @param cardAmountSFont
 *   @parent SFont Compatibility
 *   @text カード金額 SFont
 ...
 *   @default -1
 *   @min -1
 *   @desc カードライブラリ/デッキエディタ内のカードの枚数を示すテキストに使用する SFont のインデックス。
 *   
 *   @param zoneAmountSFont
 *   @parent SFont Compatibility
 *   @text デッキ/墓地 SFont
 ...
 *   @default -1
 *   @min -1
 *   @desc バトル中のデッキ/墓地画像にオーバーレイされるテキストに使用するSFontのインデックス。
 * 
 * @help
 * 
 * ============================================================================
 ...
 * ============================================================================
 *
 * テキストフォーマットプラスは、カードへのテキストの描画方法を一新します。
 * 
 * カードの説明が自動的に改行されるようになりました！
 * '\'、'\BR'、'<br>'を使って手動で改行することもできますが、
 * 説明欄でエンターキーを押して改行することもできます（スペースがある限り）。
 * 
* 
 * 注意：もしこのプラグインのプラグインパラメータが、
 * コアプラグインのCardGameCombatのプラグインパラメータと重複していたり、
 * 重複した機能を果たしていたりする場合は、コアプラグインのものを無視して、
 * ここにあるものを使ってください。
* 整理しやすくするために重複しています。
 * 
 * 例:カード名のフォントサイズは、このプラグインとコアプラグインに表示されます。
 * 同じパラメータですが、このプラグインはコアのパラメータを上書きします。
 * 
 * ============================================================================
 * フォントについて
 * ============================================================================
 * 
 * YEP_LoadCustomFontsを使用している場合は、このセクションを無視して、
*そのプラグインのヘルプセクションに従ってください。
 * 
 *このプラグインは、フォント フォルダに含まれていて、
 *ファイル名がフォント名とまったく同じであれば、どんなフォントでも許可します。
 * 
 * 例えば、Comic Sans を入れたい場合、そのフォントをシステムフォントから
 * fonts フォルダにコピーし、一連のフォントファイルが含まれていることに気づくかも
 *しれません。このプラグインで正しく読み込むには、このファイルの名前を
 * 「Comic Sans MS.ttf」に変更する必要があります。
* 
 * 私の知る限り、フォントファイルの名前を変更しても問題は起きません。
 * 
 * ============================================================================
 * テキストスタイル
 * ============================================================================
 * 
 * デフォルトでは、RPG Makerはすべてのテキストにアウトラインを追加します。
 * このパラメーターは、カード上のテキストの各要素を描画する際にその動作を
 * 継続するか、またはその動作を変更するかを決めることができます。
 * 
 * オプションは以下の通り：
 *     Default - ゲーム内の他のすべてのテキストと同じ動作を使用する。
 *     Outline ? 通常のRPG Maker アウトライン テキスト描画動作を使用する
 *     Shadow - テキストの下に黒いドロップシャドウを使う
 *     None - テキストを描くだけ
 *     
 * ほとんどのユーザーにとって、DefaultとOutlineに違いはありません。
 * RPG Makerのテキスト描画方法を変更するプラグインには、アウトラインを
 * 削除するポピュラーなものなどがあります。
 * そのようなプラグインを使用している場合、Outlineはそのカード要素のみ
 * デフォルトの動作を復元します。
 * ============================================================================
 * テキストスタイル値
 * ============================================================================
 * 
 * Default/Outline および Shadow テキストスタイルに関連する数値です。
 * 
 * アウトラインを描画するときに、アウトラインの幅を表します。デフォルトは4。
 * 
 * ドロップシャドウを描画するとき、ドロップシャドウが通常のテキストから
 * オフセットされるピクセル数を指定します。
 * 推奨値はフォントサイズに応じて2です。
 * 
 * Noneの場合、面白いことは何も起こらないので、この値は使用されない。
 *
 * ============================================================================
 *カードのテキスト情報を更新
 * ============================================================================
 * 
 * 多くのゲームではカードのコストを変更したり、カードの説明文の中に
 * ダメージ計算を入れたりします。このパラメータをONに設定することで、
 * カードをプレイした時やスキルを使用した直後にカードのテキストが更新され、
 * 常に最新のテキスト情報が表示されるようになります。
 * 
 * しかし、これを行わないゲームについては、最適化措置としてこの機能をオフにする
 * オプションを与えています。カードテキストがリフレッシュされる際にフレームレート
 * がわずかに低下することに気づきましたが、私たちのテスト環境では大きな問題では
 * ありません。
* 
 * また、以下のコードを使って、すべてのカードテキストを手動で
 * リフレッシュすることもできます：
*
 *     Myth.CGC.refreshCardText();
 *     
 * これは、カード・テキストの更新がオンでもオフでも機能します。
 * 
 * 
 * ============================================================================
 * バージョン履歴
 ...
 *
 * v1.2.0 - 大幅なリファクタリング。
 *          各テキストコンポーネントのテキスト配置パラメータを追加。
 *          各テキストコンポーネントの表示を無効にするパラメータを追加。
 *          SFontインデックス0が表示されないTextFormatPlusのバグを修正。
 *	    Amountが常にインデックス0を使用するTextFormatPlusのバグを修正。
 *	    TextFormatPlusにtryParseコールを追加し、デフォルト値を指定して
 *	    エラーを防止。
 * 
 * v1.1.1 - YEP_CoreEngine とデフォルト以外のフォントを使用する場合の MV の速度低下を修正。
 * 
* v1.1.0 - CGC 1.6.0との互換性を更新しました。
 *          VE_SFont との互換性を追加。
 *          カード名がカードコストのフォントサイズを使用していたバグを修正。
 *          テキストコード使用時に、テキストが正しく折り返されないバグを修正。
 *          カラーコードが壊れてしまうMZのバグを修正。
 *          新しいプラグインパラメータにより、.woffファイルのサポートを追加。
 *          プラグイン名を変更。
 * 
 * v1.0.1 - TPとMPの両方を描画する必要がある場合は、TPとMPを隣り合わせに描画する。
 *          YEP_CoreEngine使用時の最適化に関する問題を修正しました。
 * 
 * v1.0.0 - プラグイン完成
*/
