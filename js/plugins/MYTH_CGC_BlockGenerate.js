...
* @target MZ
* @author Swarnava Banerjee (Neel)
* @plugindesc v1.2.0 Slay the Spireのスタイルでアクターの上にブロックを作る
* @url https://mythatelier.itch.io/card-game-combat
*
* ============================================================================
* パラメーター
...
* @parent Block State Settings 
* @text ブロック要素
* @desc このステートがブロックする要素（カンマ区切り）、それ以外はバイパスする。
* @type string
* @default 1
*
* @param defaultInitBlock
...
* @type number
* @text デフォルトの最大ブロック
* @desc バトル開始時の全バトラーのデフォルトの最大ブロック数。最大値を設定しない場合は-1。
* @default -1
*
* @param blockIconOffActor
* @parent Block State Settings
...
* @type struct<Coordinate>
* @default {"x":"0","y":"0"}
* @desc アクターの基本位置を基準としたブロックアイコンのXおよびYオフセット。
*
* @param blockIconOffEnemy
* @parent Block State Settings
* @text エネミーオフセット
* @type struct<Coordinate>
* @default {"x":"0","y":"0"}
* @desc 敵の基本位置を基準としたブロックアイコンの X および Y オフセット。
*
* @param blockIconFont
* @parent Block State Settings
* @text フォント
...
* @text ブロック状態メッセージ
* @type string
* @default “ブロック！%1が %2のダメージをブロックしました！”
* @desc ブロック状態が保持されているときのバトルログ、テキストポップアップ。%1 ? ターゲット、 %2 - ダメージ
*
* @param breakText
* @parent Block State Settings
* @text ブレイク状態メッセージ
* @type string
* @default “ブレイク！ %1が %2 のダメージを受けました!”
* @desc “ブロック状態解除時のバトルログ、テキストポップアップ。%1- ターゲット, %2 - ダメージ”
*
* @param Health State Settings
*
* @param healthStateID
...
* @type number
* @text ヘルスID
* @desc データベースのヘルスIDを設定します。0の場合は表示されません。
* @min 0
*
* @param healthIconOffActor
* @parent Health State Settings
...
* @type struct<Coordinate>
* @default {"x":"0","y":"0"}
* @desc アクターの基本位置を基準としたヘルスアイコンの X および Y オフセット。
*
* @param healthIconOffEnemy
* @parent Health State Settings
* @text エネミーオフセット
* @type struct<Coordinate>
* @default {"x":"0","y":"0"}
* @desc 敵の基本位置を基準としたヘルスアイコンの X および Y オフセット。
*
* @param healthIconFont
* @parent Health State Settings
* @text フォント
...
* 概要
* ============================================================================
* Block Generateは、Slay the Spireのようなデッキビルダーのスタイルで
* Blockをシミュレートするユーティリティ・プラグインです。
* Blockはカードをプレイしたり、特定のアイテムを装備したりすることで
* 蓄積される防御リソースです。アクターにダメージが与えられると、
* 体力よりも先にブロックが差し引かれます。
*
* このプラグインは、アクターやエネミーが任意の量のブロックを持っているときに
* 自動的に適用されるブロック状態として、1つの状態を選択することができます。
* このBlockが異なるソースからどのように獲得され、どのように失われるかを
* 決定するためのメモタグとスクリプトコール（下記参照）が含まれています
*
*
* ============================================================================
* カードアクション
...
*
* Add Block [+/-]X
*     カードのターゲットに X ブロックを追加する。
*     Xは負の整数でも正の整数でもよい。
*
* Set Block X
*     Xは正の整数か0でなければならない。
*     XがターゲットのMax Blockより大きい場合、値はMax Blockにクランプされる。
*     
*
* Clear Block
*     カードの対象からすべてのブロックを消す。
...
*   <Init Block: X>
*
* 戦闘開始時のBlockの初期値を設定する。
*
*   <Max Block: X>
*
* このアクター/エネミーが一度に持てるブロックの最大値を設定する。
*
*    <Hide Block>
*
* スプライトとウィンドウ上のバトラーの現在の BLK のブロックアイコンを非表示にします。
*
*    <Hide Health>
*
* スプライトとウィンドウ上のバトラーの現在のHPのテキストを含むヘルスアイコンを非表示にします。
*
* For Skills and Items:
*
*    <Ignore Block>
*
* これらのスキルやアイテムのダメージ計算式は対象のBlockを無視し、
* 代わりにHPに直接ダメージを与える。貫通タイプのスキルに有効。
*
* For Weapons and Armors:
*
*   <Mod Init Block: +x>
*
* アクターの初期ブロック量を、正負の数値を加えることで変更する。
* 複数のソース(異なる装備品のような)からスタックすることができる。
*
*   <Mod Max Block: +x>
*
* アクターのMax Blockに正負の数値を加えることで、Max Blockの量を変更する。
* 複数のソース(異なる装備品のような)からスタックすることができる。
*
* For States:
*
*    <Block Change: +X>
*
*  ターンの終了時(再生ステップ中)に、その状態を持つアクターが
* どれだけブロックを得るか/失うかを決定する。複数のソースから
* スタックすることができる。
* 例えば、状態Aが毎ターン20ブロックを獲得し、
* 状態Bが毎ターン40ブロックを失う場合、状態Aと状態Bの両方を持つアクターは
* 毎ターン20ブロック(+20 - 40 = -20)を失う。
*
*    <Negate Block>
*
* このノートタグを持つ状態がアクターに適用された場合、
* そのアクターはすべてのブロックを失い、経過するか削除されるまで
* ブロックを得ることができなくなる。
* ============================================================================
* スクリプトコール
* ============================================================================
...
*   {battler}.setBlock(value, show)
*
* バトラーズ・ブロックの値を設定します。Showはダメージポップアップとバトルログ
* メッセージに変化を表示させたいかどうかによってtrue/falseを指定できます。
*
*   {battler}.addBlock(value, show)
*
* バトラーズブロックに加える。値は正でも負でもよい。
* Battler's Maxを超えるか0を下回ると、値は自動的にクランプされる。
* ダメージポップアップとバトルログメッセージのどちらに変更を表示させたいかに
* よって、Showをtrue/falseにできます。
*
*   {battler}.clearBlock(show)
*
* バトラーのブロックを0にし、ブロック状態を解除する。
*
* Usage Examples:
*
* $gameParty.leader().getBlock();
...
* ============================================================================
*
* v1.2.0  - 通常攻撃のダメージタイプがブロックを無視する問題を修正しました。
*         - <Ignore Block>が再び機能し、ダメージタイプの影響を受けなくなりました。
*         - ブロックの追加、セット、ブロックを消すカードアクションを追加しました。
*         -  ブロックがクリアされたにもかかわらず、ラウンドを持ち越すバグを修正しました。
*
*
* v1.1.0 - バトラーにヘルスとブロックの両方を表示したい場合に備えて、ヘルスアイコンを追加しました。
*         ブロックの両方を表示できるようにしました。非表示にしたい場合は、メモタグがあります。
*        - ブロックとヘルスの値は、アクターとパーティーのウィンドウのアイコンに書き込まれます。
*          ブロックとヘルスのアイコンのテキストに異なるフォントを設定できます。
*        - ブロックとブレイクの状態メッセージのテキストを公開。
*          カスタマイズ可能で、MVとMZの違いがあります。
*        - エレメントのサポートを追加。ブロックステートが軽減するダメージタイプと
*       　 無視するダメージタイプを選択できるようになりました。

*        - アクターとエネミーに対して、異なるアイコンオフセットを宣言することができます。
*
* v1.0.0 -  基本機能完成！CGCデモ・プロジェクトでのテストが終わったら、
* また細かい編集をする予定です。
*
*
* =============================================================================
...
* =============================================================================
*
* このツールはMythAtelier LLCによって開発されました。
*
* あなたのプロジェクトにもっとツールが必要ですか？
* 他のプラグインもぜひご覧ください：
* https://itch.io/c/1695699/tools-plugins
*
* 不具合報告などはこちら
* https://discord.gg/wRk4XHF5tZ
*
*もしこのプラグインが気に入り、私たちをサポートしたいと思ったら、
*ぜひPatreonをご覧ください
* https://www.patreon.com/mythatelier
*
*/

/*~struct~BlockState:ja
* @param stateId
* @text ステート ID
* @type number
* @desc ブロック状態のステートID。
...
* @param blockedTypes
* @text ブロックされた要素
* @desc このステートがブロックする要素のリスト（カンマで区切る）。
* @type string
* @default "1"
*
*/
