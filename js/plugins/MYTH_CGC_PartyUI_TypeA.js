 ...
 * @target MZ
 * @author Isiah Brighton + Swarnava Banerjee (Neel)
 * @plugindesc v1.1.2 MYTH_CGC_CoreEngineと組み合わせて使用するUIプラグインです。
 * @url https://mythatelier.itch.io/card-game-combat
 * 
 * @param Party Status Window
 *
 ...
 * @text Micro Status Window Width	
 * @text 見せる／隠す？
 * @default true
 * @desc True の場合、パーティーステータスウィンドウを表示します。False の場合、非表示になります。
 *
 * @param showActorPortParty
 * @parent Party Status Window
 * @type boolean
 * @text アクターのフェイス画像を使う？
 * @default true
 * @desc True の場合、アクターのフェイス画像をパーティーステータスウィンドウに表示します。False の場合は非表示になります。
 *
 * @param partyStatusOffset
 * @parent Party Status Window
 * @type struct<Coordinate>
 * @text XY オフセット
 * @default {"x":"0","y":"0"}
 * @desc パーティーステータスウィンドウのオフセット位置。画面右上に固定されています。
 *
 * @param partyStatusWidth
 * @parent Party Status Window
 * @type number
 ...
 * @text Height
 * @default 192
 * @desc メインステータスウィンドウの高さ
 *
* @param partyStatusSkin
* @parent Party Status Window
* @text ウィンドウスキン
* @desc パーティーのステータスウィンドウのスキン(空欄の場合はデフォルトのまま)
* @type file
* @dir img/system
* @require 1
 *
 ...
 * @text 見せる／隠す？
 * @default true
 * @desc Trueの場合、アクターステータスウィンドウを表示。False なら非表示。
 *
 * @param showActorPortActor
 * @parent Actor Status Window
 * @type boolean
 * @text アクターのフェイス画像を使う？
 * @default true
 * @desc Trueの場合、アクターステータスウィンドウにアクターのフェイス画像を表示します。False の場合は非表示。
 *
 * @param actorStatusOffset
 * @parent Actor Status Window
 * @type struct<Coordinate>
 * @text XY オフセット
 * @default {"x":"0","y":"0"}
 * @desc アクターステータスウィンドウのオフセット位置。画面左下に固定されます。
 *
 * @param actorStatusWidth
 * @parent Actor Status Window
 * @text 幅
 ...
 * @text 高さ
 * @default 180
 * @desc アクターステータスウィンドウの高さ
 * 
 * @param actorStatusSkin
 * @parent Actor Status Window
 * @text ウィンドウスキン
 * @desc アクターステータスウィンドウのスキン(空欄の場合はデフォルトのまま)
 * @type file
 * @dir img/system
 * @require 1
 *
 ...
 * ============================================================================
 * 
 * このプラグインは、バトルシーンに2つの異なるステータスウィンドウを提供します：
 * 1. パーティーステータスウィンドウ
 *     これは右上に表示されるウィンドウで、パーティーのステータスを表示します。
 *     
 * 2. アクターステータスウィンドウ
 *     これは左下に表示されるウィンドウで、現在のアクターのすべてのステータスが表示されます。
 * 
 * このプラグインは MYTH_CGC_CoreEngine の直下に配置してください。 
 * 
 * 
 * バトルUIに影響するプラグインパラメータはMYTH_CGC_CoreEngineの中にあります。もし変更したいパラメータがこのプラグインにない場合は、そちらをチェックしてください。
 * 
 * ============================================================================
 * バージョン履歴
 * ============================================================================
 * 
 * v1.1.2 - パーティーステータスウィンドウが無効になっていると、
 *        ハンドのアップボタンを押すとソフトロックするバグを修正。
 *          
 * 
* v1.1.1 - バトルログとヘルプウィンドウが切れてしまうバグを修正
* - アクターとパーティーステータスウィンドウを個別のウィンドウレイヤーに
* - アクターの肖像画が境界外に出ないよう修正
* - ステータスオフセットウィンドウが機能しないバグを修正
* - アクターのステータスUI要素をよりわかりやすく変更
* - アクターとパーティーステータスウィンドウのTPゲージを修正
* - アイテムウィンドウの高さを変更し、パーティステータスと重ならないように修正
* - アクターとパーティーステータスウィンドウにウィンドウスキンのパラメータを追加
*
* v1.1.0 - プラグインの名前を変更しました。
 * - わかりやすくするため、「マイクロステータスウィンドウ」を「パーティーステータスウィンドウ」に変更
 * - わかりやすくするため、「メインステータスウィンドウ」を「アクターステータスウィンドウ」に変更。
 * - アクターステータスウィンドウのXオフセットが機能していなかったバグを修正。
 * - ウィンドウ枠をMZウィンドウに戻し、レクトの位置を調整しました。
 *
 * 
 */
