 ...
 * @target MZ *
 * @author Isiah Brighton
 * @plugindesc v1.0.4 MYTH_CGC_CoreEngineの拡張で、装備品やステートがアクターのデッキにカードを追加できるようになりました。
 * @url https://www.patreon.com/mythatelier
 *
 *
 * @help
 ...
 * ============================================================================
 *
 * このプラグインを使うと、装備やステートを使ってアクターのデッキを
 * 操作することができます。
 *
 * ============================================================================
 * 武器/防具/ステート メモタグ
 ...
 * <Add Card X>
 * 
 * X を、アクターがデッキで獲得するスキルの ID を表す数字に置き換えてください。
 * 
 * 
 * アクターがこのタグを持つ武器や防具を装備すると、このスキルはデッキに
 * シャッフルされます。このタグを持つステータスを与えられた場合、
 * そのスキルはデッキの一番上に追加されます。
 *
 * アクターが戦闘中にステータスを失ったり、装備を変更した場合、
 * そのカードはデッキから取り除かれます。
 * デッキにない場合は、手札あるいは墓地から取り除かれます。
 * 
 * <Add Card [skill name]>
 * 
 * スキルIDに加えて、名前でも追加できます。大文字と小文字を区別します。
 *
 * ============================================================================
 * バージョン履歴
 * ============================================================================
* v1.0.4 - DeckEditorCore v1.2.0との互換性を確保するためのアップデート
 * 
 * v1.0.3 - 部隊戦をテストすると、装備品から得られるカードが
 *         2倍になるという稀なバグを修正。 
*
 * v1.0.2 - CGC v1.5.1との互換性のために更新されました。
 *         unequip から削除されたカードは、equip から追加されたカードと
 *         全く同じインスタンスになりました。

 * v1.0.1 - ヘルプセクションを修正。
 *         スキル名によるカードの追加を追加しました。
 *         装備開始時にカードも追加されるようになりました。
 * 
 * v1.0.0 -プラグインをリリースしました。
 *
 */
