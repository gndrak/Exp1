 ...
 * @target MZ
 * @author Isiah Brighton
 * @plugindesc v1.6.2 スキルを カードに変換し、バトルで使用できるようにします。
 * @url https://mythatelier.itch.io/card-game-combat
 * 
 * @============================================================================
 * Commands
 ...
 *   @type bool
 *   @default true
 *   @desc ONの場合、カードバトルになります。オフにすると、デフォルトのバトルになります。
 *
 * @command setCardBattle
 * @text カードバトルをセット
 * @desc カードバトルが有効かどうかを決定します。
 *
 *   @arg enabled
 *   @type boolean
 *   @default true
 ...
 * @command setCardLibrary
 * @text カードライブラリーをセット
 * @desc カードライブラリーをプレイヤーのメニューに表示するかどうかを決定します。
 *
 *   @arg enabled
 *   @type boolean
 *   @default true
 *   @desc ONの場合、カードライブラリーがプレイヤーのメニューに表示されます。
 *
 * @command openCardLibrary
 * @text カードライブラリーを開く
 * @desc カードライブラリーを開きます。
 ...
 *     @type number
 *     @default 0
 *     @desc 非表示にするカードのインデックス
 *
 * @command HideCardID
 * @text スキルIDによるカードの非表示
 *
 *     @arg skillID
 *     @type number
 *     @default 1
 *     @min 1
 *     @desc 非表示にするカードのスキルID。同じスキルのカードが画面上に複数ある場合、インデックスが最も高いカードが非表示になります。
 *
 * @command MoveCard
 * @text カードの移動
 *
 ...
 *     @default 0
 *     @min -100
 *     @desc 移動するカードのy座標。カード画像はその座標を中心に配置される。
 *
 *
 * @command HideAllCards
 * @text Hide All Cards
 ...
 * @param startOfBattleActions
 * @text 戦闘開始アクション
 * @desc アクターの最初のターン／戦闘開始時に起こるアクションのリストで、改行で区切られています。
 * @type note
 * @default "Shuffle Deck\nDraw 5"
 * 
 * @param startOfTurnActions
 * @text ターン開始アクション
 * @desc プレイヤーの2ターン目以降の開始時に起こるアクションを、改行で区切ったリスト。
 * @type note
 * @default "Discard Until 0\nDraw 5"
 * 
 * @param maxHandSize
 ...
 * @default -1
 * @min -1
 * @desc アクターがドローできなくなるまでの手札の最大枚数。手札が無限の場合は-1に設定します。
 * 
 * 
 * @param Card Appearance
 * 
 ...
 *   @type number
 *   @default 34
 *   @desc Costは右寄せなので、Costスペースの幅。これ＋X＝テキストの一番右側。
 *        
 *   @param cardDescCoords
 *   @text カードの説明座標
 *   @parent Card Appearance
 *   @type struct<Coordinate>
 *   @default {"x":"4","y":"100"}
 *   @desc カードの説明がカード上で描画を開始する座標。
 *  
 *   @param cardHighlightSheet
 *   @text ハイライトシート
 *   @parent Card Appearance
 *   @default CardHighlightSheet
 *   @desc img/CGC/highlightsにあるカードハイライトの画像シート。1列目はスタンダード、2列目はディスカード、3列目はリムーブです。
 *   
 *   @param removalAnimation
 *   @text 除去アニメーション
 *   @parent Card Appearance
 ...
 *   @type boolean
 *   @default true
 *   @desc カードの拡大／縮小を滑らかにするか（ON）、ピクセル化するか（OFF）。
 * 
 * @param buttonOrder
 * @text UIの順序
 * @type string
 * @default Item Hand End
 * @desc ボタン/ゾーンの名前を、左から右にアクセスしたい順番に並べます。デフォルトは「Item Hand End」です。
 *
 * @param Zones
 * @text ゾーン
 * @parent buttonOrder
 ...
 *   @type struct<Zone>
 *   @default {"name":"Deck","sheet":"","coordinates":"{\"x\":\"48\",\"y\":\"380\"}","cardScale":"0.5000","skew":"{\"x\":\"0.0000\",\"y\":\"0.0000\"}","cardRotation":"-15","amountFont":"24","amountCoordinates":"{\"x\":\"32\",\"y\":\"48\"}","showCards":"No","description":"\"Check contents of Card Deck\"","enterSFX":"","exitSFX":"","emptyActions":"\"Move all from discard to deck\\nshuffle deck\""}
 *   @desc デッキの外見や戦闘中の行動のパラメータ。名前は変更しないでください。
 *   
 *   @param discardZone
 *   @parent Zones
 *   @text 廃棄ゾーン
 *   @type struct<Zone>
 *   @default {"name":"Discard","sheet":"","coordinates":"{\"x\":\"752\",\"y\":\"380\"}","cardScale":"0.5000","skew":"{\"x\":\"0.0000\",\"y\":\"0.0000\"}","cardRotation":"15","amountFont":"24","amountCoordinates":"{\"x\":\"32\",\"y\":\"48\"}","showCards":"Behind image","description":"\"Check contents of Discard Pile\\nDo what must be done.\"","enterSFX":"","exitSFX":"","emptyActions":""}
 *   @desc ディスカードの外見と戦闘中の行動のパラメータ。名前は変更しないでください。
 * 
 * 
 *   @param extraZones
 *   @parent Zones
 ...
 *   @text 臨時ゾーン
 *   @type struct<TempZone>
 *   @desc カードの効果が解決されている間にカードが移動する場所。本当のゾーンではない。
 *   @default {"coordinates":"{\"x\":\"664\",\"y\":\"380\"}","cardScale":"0.5000","skew":"{\"x\":\"0.0000\",\"y\":\"0.0000\"}","cardRotation":"15"}
 *   
 * @param Battle Buttons
 * @parent buttonOrder
 ...
 *   @param endTurnButtonStruct
 *   @text ターン終了ボタン
 *   @descボタンオーダーに「End」が含まれていない場合は無視してください。ターン終了ボタンは表示されません。
 *   @type struct<EndTurnButton>
 *   @parent Battle Buttons
 *   @default {"skillID":"2","endTurnSheet":"endTurnSheet","discardSheet":"endTurnDiscardSheet","coordinates":"{\"x\":\"760\",\"y\":\"580\"}","enabledCondition":"true"}
 *
 *   @param itemButtonStruct
 *   @text アイテムボタン
 *   @desc Button Orderに'Item'が含まれていない場合は無視してください。アイテムボタンは表示されません。
 *   @type struct<ItemButton>
 *   @parent Battle Buttons
 *   @default {"imageSheet":"itemSheet","coordinates":"{\"x\":\"760\",\"y\":\"500\"}","description":"Open the Item Menu","enabledCondition":"true"}
 *
 ...
 * @type boolean
 * @default true
 * @desc バトル ステータス ウィンドウやアクター コマンド ウィンドウを補強する他のプラグインがある場合は、これをオフに設定してください。
 * 
 *   @param showHelpWindow
 *   @parent changeBattleWindows
 *   @text ヘルプウィンドウの表示
 *   @type boolean
 *   @default true
 *   @desc カードの説明を表示する場合、スキルのヘルプウィンドウをオフにすることができます。その他のヘルプウィンドウは表示されます。
 *   
 *   @param skipPartyCommand
 *   @parent changeBattleWindows
 *   @text スキップパーティーコマンドメニュー
 *   @type boolean
 *   @default true
 *   @desc ONに設定すると、プレイヤーはパーティコマンドから始めるのではなく、アクターコマンドウィンドウに直行します。

 *   
 *   @param skipActorCommand
 *   @parent changeBattleWindows
 *   @text アクターコマンドメニューをスキップする
 *   @type boolean
 *   @default true
 *   @desc ONに設定すると、プレイヤーはアクターコマンドウィンドウから始めるのではなく、そのままカード選択に入ります。
 *   
 *   @param statusWindowTop
 *   @parent changeBattleWindows
 *   @text 上部のステータスウィンドウ
 *   @type boolean
 *   @default true
 *   @desc ONに設定すると、ステータスウィンドウ（とコマンドウィンドウ）がヘルプウィンドウの下に表示されます。
 *   
 *   @param displayStatusCardIcons
 *   @parent changeBattleWindows
 *   @text ディスプレイカードのアイコン
 *   @type boolean
 *   @default true
 *   @desc ONに設定すると、バトルステータスウィンドウに追加アイコンが表示され、各アクターが各ゾーンに何枚カードを持っているかが表示されます。
 *   
 *     @param handSizeIcon
 *     @parent displayStatusCardIcons
 *     @text ハンドアイコン インデックス
 ...
 *     @type number
 *     @default 2
 *     @desc アクターのデッキを表すアイコンのインデックス。
 *     
 *     @param discardSizeIcon
 *     @parent displayStatusCardIcons
 *     @text 破棄アイコン インデックス
 *     @default 3
 *     @desc アクターの破棄を表すアイコンのインデックス。
 *   
 *   
 * @param General Sounds
 * 
 ...
 *   @text カード追加SE
 *   @parent General Sounds
 *   @desc カードがゾーンに追加された時（作成された時）に再生されるSE
 *   @type file
 *   @dir audio/se/
 *   @require 1
 *   
 ...
 *   @text カード除去時のSE
 *   @parent General Sounds
 *   @desc カードがプレイから取り除かれたときに再生されるSE
 *   @type file
 *   @dir audio/se/
 *   @require 1
 *   
 ...
 *   @text シャッフル SE
 *   @parent General Sounds
 *   @desc デッキカードのシャッフルアクションが実行されたときに再生されるSE
 *   @type file
 *   @dir audio/se/
 *   @require 1
 *   
 * @param Card Selection Window
 * @desc バトル中にデッキやディスカードのカードを表示するウィンドウ。
 *   
 *   @param cardSelectionWindowskin
 *   @parent Card Selection Window
 *   @text ウィンドウスキン
 *   @desc カードを表示するウィンドウのウィンドウスキン。
 *   @type file
 *   @dir img/system
 *   @default Window
 *   @require 1
 ...
 *   @parent Card Selection Window
 *   @text ウィンドウY
 *   @desc カードを表示するウィンドウのY位置。ウィンドウを垂直方向の中央に配置したい場合は-1を設定する。
 *   @type number
 *   @default -1
 *   
 *   @param cardSelectionHeight
 ...
 *   @option No Card Library in Menu
 *   @default Replace Skill Menu
 *   @desc カードライブラリーがメニューにあるかどうかを決定する。プラグインコマンドでオーバーライド可能。
 *   
 *   @param cardLibraryMenuDesc
 *   @parent Card Library
 *   @text カードライブラリーのメニューテキスト
 *   @desc カードライブラリオプションがメニューに追加されたときに表示されるテキスト。スキルコマンドテキストの代わりにはなりません。
 *   @default Cards
 *   
 *   @param simpleView
 *   @parent Card Library
 *   @text シンプルビュー
 *   @desc ONに設定すると、アクターのカードはそれぞれ別々に表示されます。
 *   @type boolean
 *   @default false
 *   
 *   @param showMissingCards
 *   @parent Card Library
 *   @text ライブラリーで消えたカードを表示する
 *   @desc これがオンに設定されている場合、欠落したカードはライブラリーのシーンでグレーアウトして表示されます。
 *   @type boolean
 *   @default false
 *   
 *   @param showGapCards
 *   @parent Card Library
 *   @text ライブラリーでギャップカードを見せる
 *   @descこの設定がオンの場合、最も低いスキルIDのカードと最も高いスキルIDのカードの間に欠けているカードはグレーアウトして表示されます。
 *   @type boolean
 *   @default false
 *   
 *   @param showBlankCards
 *   @parent Card Library
 *   @text ライブラリーに空白の名前のカードを表示する
 *   @descこの設定をONにすると、スキルシーンに空の名前のカードが表示されます。
 *   @type boolean
 *   @default false
 *
 *   @param displayOriginBadges
 ...
 *   @default true
 *   @parent Card Library
 *   @desc ライブラリーのカードに表示される画像で、カードが学習によって獲得されたものなのか、装備によって獲得されたものなのか、ステートによって獲得されたものなのかを表示します。
 * 
 *      @param cardOriginCoords
 *      @text バッジ座標
 *      @parent displayOriginBadges
 *      @type struct<Coordinate>
 *      @desc オリジンバッジが描かれるカードの場所
 *      @default {"x":"170","y":"300"}
 *      
 *      @param learnedBadge
 *      @text スタンダード
 ...
 *      @type file
 *      @dir img/system
 *      @desc 従来の方法で獲得したカードに表示されるイメージ。使用しない場合は空白にしてください。
 *      @default learnedBadge
 *      
 *      @param equipBadge
 *      @text 装備カード
 ...
 *      @type file
 *      @dir img/system
 *      @desc 装備して得たカードに表示される画像。使用しない場合は空白にしてください。
 *      @default equipBadge
 *      
 *      @param stateBadge
 *      @text ステートカード
 ...
 *      @type file
 *      @dir img/system
 *      @desc ステートで獲得したカードに表示される画像。使用しない場合は空白にしてください。
 *      @default stateBadge
 *      
 *      
 *      
 ...
 * 
 * <Hide from card library>
 *     カードライブラリーにカードが表示されなくなる
 *     
 * <Card Art: file_name>
 *     カードのカードアートを指定された名前のファイルにする。
 *     アートはプラグインパラメーターのartディレクトリにあります。
 *     
 * <Card Base: file_name>
 *     デフォルトのカードBaseを、指定された名前のファイルで置き換えます。
 *     Baseはプラグインパラメータのbaseディレクトリにあります。
 *     
 * 
 *     
 * In Skills and Items:
 ...
 * 
 * Draw X
 *     Xは数字で、このカードをプレイするとX枚のカードを引きます。
 *
 * Draw Until X
 *     Xは数字で、手札の枚数がXになるまでドローを行います。
 *
 * Discard X
 *     これをプレイするために、そのプレイヤーにX枚のカードを捨てさせる。
 *
 * Discard Until X
 *     手札の枚数がXになるまで捨て札をさせます。
 *     
 * Remove X
 * Remove Until X
 *     選択したカードはプレイから除外される。
 *     
 * Add [skillID] to [zoneName]
 *      カードを直接ゾーンに追加します。例えば、「7を手札に加える」と、
 *       スキルIDが7のカードを手札に加えます。
 *
 * Mill X
 *     山札から X 枚のカードを直接捨て札に移す。
 *     
 * Search For X
 * Search For [skill name]
 *     指定されたスキルID(x)または名前のカードを引きます。
 *     
 *     
 * Move X from [zoneName] to [zoneName]
 *     これはドローやディスカード、ミルの代わりとなるものですが、
*     全てのゾーンで機能します。
 *     
 * Move all from [zoneName] to [zoneName]
 *     これにより、すべてのカードがあるゾーンから別のゾーンに移動する。
 * 
 * Wait X
 *     カードアクションの実行を、RPGツクールのWaitコマンドのように
*     Xフレーム待たせる。Start of Turn/Battleパラメータでは動作しません。
 *     
 * Shuffle Deck
 *     デッキをシャッフルする
 *    
 *    
 * Move this to [zoneName]
 *     現在使用中のカードを指定されたゾーンに移動します。これが指定されていない
 *     場合、カードはすべてのカードアクションの終了時に捨て札に移動します。
 * 
 * Remove this
 *     現在使用されているカードをプレイから取り除く。
 * 
 * Play SE [fileName]
 * Play SFX [fileName]
 *     audio/seにある指定されたサウンドファイルを再生します。
 *    
 *    
 * Eval [expression]
 *     “eval "以降をコードとして実行する。
 *    
 * If [expression]
 *     式を評価し、それが真であれば次のカードアクションを実行します。
 *     偽の場合、そのアクションはスキップされます。
 *    
 * Label [labelName]
 *     これは次のカードアクションにとって重要です。
 *    
 * Jump to [labelName]
 *     同じ名前のラベルを探し、そのラベルに移動する。
 *     このカードアクションの後にあれば、その間のすべてのカードアクションを
 *     スキップします。もしラベルがこのカードアクションの前にあれば、
 *     2回目のカードアクションを実行する。
 *     
 * Force Action [skillID]
 *     アクターやターゲットに指定したスキルを実行させます。
 *    
 * Passives:
 * 
 * Will End Turn
 *     これにより、このカードは使用時にターンを終了する。
 *     
 * Discard if unplayed
 *     このカードが手札にある状態でターンが終了した場合、このカードを捨てる。
 *     
 *     
 * Remove if unplayed
 *     このカードが手札にある状態でターンが終了した場合、
 *     このカードをプレイから除外する。
 *     
 * Start in [zoneName]
 *     戦闘開始時、戦闘開始アクションの前に指定されたゾーンにいます。
 *     
 *     
 * Require [expression]
 *     式が真と評価されない限り、プレーすることはできない。
 *     複数の要件を置くことができる。
 *     カードの使用者を指すには「user」を使用する。
 *     
 *     ex:
 *     
 *     Require user.handSize > 2
 ...
 *     Require $gameSwitches.value(3)
 *     
 *     手札が2枚以上10枚以下の場合のみプレイ可能になります。
 *     また、ゲームスイッチ3がONの場合のみプレイ可能です。
 *     
 *
 * Start of Turn: Skill X
 *      ターン開始時、このカードが手札にある場合、スキルID Xを実行する。
 *
 * End of Turn: Skill X
 *       ターン終了時、このカードが手札にある場合、スキルID Xを実行する。
 *      
 * Enter [zoneName]: Skill X
 *      このカードが指定したゾーンに入ったとき、スキルID Xを実行する。
 *      
 * Exit [zoneName]: Skill X
 *      このカードが指定されたゾーンから出るとき、スキルID Xを実行する。
 * 
 * 
 * ============================================================================
 * プラグインコマンド
 * ============================================================================
 * 
 * DisableCardBattle - カードによる戦闘を行わない状態に戻す。
 * EnableCardBattle - カード戦闘に戻す。
 * これらのコマンドは戦闘の外で呼び出す必要があります。
 * 
 * EnableCardLibrary - メニューにカードライブラリオプションを追加します。
 * DisableCardLibrary - メニューからカードライブラリオプションを削除します。
 * 
 * OpenCardLibrary - カードライブラリのシーンを直接開きます。
 * 
 * 
 * ShowCard [ID] [x] [y] [angle] [index]
 * 
 * 指定されたカードを画面に表示する。
 * 括弧をそれぞれのフィールドに置き換える。IndexはShow Pictureの時のNumberと
 * 同じように動作します。
 *
 * Indexが省略された場合、新しいIndexを見つけ、新しいカードを作成します。
 * 角度が省略された場合、角度は0になります。
 *
 * HideCardID [ID]
 *
 ...
 * MoveCard [index] [x] [y]
 *
 * 指定されたインデックスのカードを、指定された x 座標と y 座標に移動します。
 * 
 *
 *
 *
 * 注：index以外のパラメータは生の数値の代わりに、
 * ゲーム変数(Game Variable)を入力できます。
 *
 * Ex:
 *     ShowCard \V[4] \V[5] \V[6] \V[7] 0
 * ゲーム変数4にIDが格納されているカードを、変数5～6に格納されている座標に、
 * 変数7に格納されている角度で表示し、そのインデックスを0に設定し、
 * そのインデックスにあったカードを置き換える。
 * 
 * 
 * ============================================================================
 * より詳細な情報やその他の機能については、このプラグインと
 * 関連するすべてのプラグインのために作られたwikiをご覧ください：
 * http://card-game-combat-help.alwaysdata.net/
 * ============================================================================
 * 
 *
 ...
 * ============================================================================
 * 
 * v1.6.2 - ShowCard をコアエンジンに統合。
* Text Format Plus に存在する重複パラメータをいくつか削除
*
* コスト X 座標を左揃えに。 これを補うため、コスト幅パラメータを追加。
* ハイライトスプライトをハイライトシートに置き換え、Remove バリエーションを追加。
* 終了ターンボタンの Remove バリエーションを追加
* ボタン画像を img/CGC/buttons に移動
* ハイライト画像をimg/CGC/highlightsに移動
* カードの由来を表示する「Origin Badges」をカードライブラリーに追加
* ライブラリーに表示するカードを決定するパラメータをいくつか追加
* 異なる由来のカードがライブラリーに個別に表示されるように
* デフォルトのカードが起動時に事前読み込みされるようになり、ほぼすべてのカードベースの計算がこの画像に基づいて実行されるようになりました。
* ライブラリー内の金額テキストが新しいクラスで処理されるようになりました 
* Window_CardAmountにより、より柔軟性が向上しました。
* カードスプライトのヒットボックスが修正され、イメージのスケールに合わせて拡大縮小されるようになりました。
* ゾーンスプライトが使用されないカードハイライトを作成しなくなりました。
* カードが適切に中央揃えされない手札の位置決めにおける計算エラーが修正されました。
* 新しいパラメータ「Skip Party Command」により、パーティーをスキップできるようになりました
* コマンドウィンドウ
* 「End of Turn: Skill X」が2回トリガーされるバグが修正されました
* ターン開始:スキルXが適切にトリガーされないバグを修正。
* 戦闘中に習得したスキルは、アクターのライブラリーに保存されるようになりました。
* アクターが装備やステータスによって獲得したカードを「忘れる」バグを修正。
* ライブラリシーンが全面的に改良されました。UIも多数改善されています。
* ライブラリタイプのシーンすべてにスクロールバーが追加されました。
* カードバトルが無効になっていると、カードスプライトがライブラリーに表示されないバグを修正
* バトルが無効になっている場合、カードスプライトがライブラリーに表示されないバグを修正
* オープニングハンドが描画される前にパーティウィンドウを使用してバトルから逃げ損ねた場合のクラッシュを修正
*
* バックエンドの大幅な改良
* エラー処理とユーザーへの通知の改善
* Window_BattleSkill固有の動作をWindow_SkillListから削除
*
* カードは、WindowLayerから動作を継承したCardLayerオブジェクトに追加されるようになりました。
* カードのハイライトスプライトは、基本的なスプライトオブジェクトではなく、新しいクラスであるSprite_CardHighlightになりました。信じてください。コードはよりすっきりしました。
* CoreEngineは10,000行を超えました。 
*
* 
 * 
* v1.6.1 - Skip Party Command Menuプラグインパラメータを追加。
 * ゾーン名の大文字と小文字が正しく区別されないバグを修正。
 * ゾーンスプライトの上にカーソルを置くと、MZタッチ入力がクラッシュする不具合を修正。
 * MZプラグインのコマンドが動作しないのを修正。
 * ターゲットが利用可能でないとき、スキルが強制的にアクションを起こすとクラッシュするのを修正。
 * 戦闘中のアイテム選択で、ステータスウィンドウが表示/非表示になるMZのバグを修正。
 *
 * v1.6.0 - ゾーンパラメータを構造体に変更。
 * カスタムゾーンを追加しました。
 * Will End Turnがカードパッシブになりました。
 * Remove thisがカードアクションになりました。
 * 新しいカードアクションです： 新しいカードアクション: Move this to [zoneName], 現在使用中のカードを指定されたゾーンに移動します。
 * 使用済みカードの解決方法が変更されました。カードは手札から取り除かれ、その効果がプレイされた後、捨札に移されます。早くカードを移動させるには「Move this」CAを使用してください。
 * デッキとディスカード・スプライトは個別の画像ではなく、シートを使用するようになりました。カスタムゾーンも同様です。
 * 手札からディスカードに置かれ、その後デッキに再シャッフルされるカードは、重複して表示される代わりに、手札から直接デッキに移動するようになりました。
 * 自動再シャッフルに代わり、ゾーンが空になった時の新しいCAパラメータが追加 されました。これらのCAはゾーンが空になった時に実行され、ゾーンがまだ空 の場合はアクション終了時に再度実行されます。
 * End Turn ButtonとItem Buttonのパラメータが構造体になりました。
 * IsiahCGCCardSelection と IsiahCGCConditionalCardActions はメインプラグインに統合されました。パラメータは必要に応じて移動、追加されました。
 * IsiahCGCExtraButtonsはメインプラグインに統合されました。
 * たくさんのパラメーターが変更されました。
 * 新しいカードアクション： SE [soundName] を再生。
 * ゾーンは、カードが出入りするときのサウンドエフェクトパラメータを持ちます。
 * 他のいくつかのSEのための新しいパラメーター。
 * スムーズなスケーリングのための新しいパラメータ。
 * アクターコマンドメニューのスキップパラメータがPartyUI_TypeAとうまく機能するようになりました。
 * 大文字と小文字の区別を削除しました。
 * MZでライブラリをスクロールする際のタッチ入力のバグを修正。
 * 強制アクションの後、ターンが正しく終了しないバグを修正。
 * MZでカードの文字色が変わってしまうバグを修正。
 * MZの余分なボタンによるタッチ入力のバグを修正。
 * バックエンドのリファクタリング。
 * レガシー関数を削除。
 * IsiahCardGameCombatからMYTH_CGC_CoreEngineにリネーム。
 * 
 * 
 * フルバージョンの履歴はWikiで閲覧できます。
*/

/*~struct~EndTurnButton:ja
 * @param skillID
 * @text ターン終了時のスキルID
 * @type skill
 * @default 2
 * @desc ターン終了ボタンを押したときに実行されるスキル
 *
 * @param endTurnSheet
 * @text ターン終了イメージシート
 * @desc エンドターンボタンの画像シートは、img/CGC/buttonsにあります。
 * 
 * @param discardSheet
 * @text 破棄イメージシート
 * @desc ボタンを廃棄状態にした場合の画像シートは、img/CGC/buttonsにあります。
 * 
 * @param removeSheet
 * @text 除外イメージシート
 * @desc ボタンの「削除」状態の画像シートは、img/CGC/buttonsにあります。
 * 
 * @param coordinates
 * @text 座標
 * @type struct<Coordinate>
 * @desc ボタンが画面上に表示される座標
 * @default {"x":"760","y":"580"}
 * 
 * @param enabledCondition
 * @text 利用条件
 * @type text
 * @default true
 * @desc ボタンが使用されるためには、この式が「真」と評価されなければなりません。
 * 
 */

/*~struct~ItemButton:ja
 *
 * @param imageSheet
 * @text スプライトシート
 * @desc アイテムボタンの画像シートは、img/CGC/buttonsにあります。
 * 
 * @param coordinates
 * @text 座標
 * @type struct<Coordinate>
 ...
 * @text 説明
 * @type text
 * @desc アイテムボタンがハイライト表示された際にヘルプウィンドウに表示されるテキスト。
 * @default Open the Item Menu
 *
 * @param enabledCondition
 * @text 利用条件
 ...
 * @param sheet
 * @text イメージシート
 * @desc ボタン用の画像シートは、img/CGC/buttonsにあります。左の画像は有効、中央の画像は無効、右の画像はハイライトです。
 * 
 * @param coordinates
 * @text 座標
 * @type struct<Coordinate>
 ...
 * @text 表示条件
 * @type text
 * @descボタンがバトルに表示されるためには、この式が必ず「真」と評価されなければならない。開始時にのみ評価される。
 * @default true
 *
 * @param disableDuringNonInput
 * @text 非入力時は無効にする
 ...
 * @dir img/system
 * @require 1
 * @desc ゾーンの外観用のイメージシート。左側に有効なイメージを配置し、右側にハイライトを配置します。
 *
 * @param coordinates
 * @text ゾーン座標
 * @type struct<Coordinate>
 * @desc ゾーンが画面上のどこに表示されるか
 *
 * @param cardScale
 * @text カードスケール
 * @type number
 * @decimals 4
 * @desc このゾーンに移動すると、スケールカードが変化します。
 * @default 0.5
 * 
 * @param skew
 * @text 歪み
 * @type struct<Point>
 * @desc 等角3D回転をシミュレートするのに使用します。
 * @default {"x":"0.0000","y":"0.0000"}
 * 
 * @param cardRotation
 * @text カードローテーション
 ...
 * @type number
 * @default 24
 * @desc このゾーンのカードの枚数を示すテキストのフォントサイズ。
 * 
 * @param amountCoordinates
 * @text 金額テキスト座標
 * @type struct<Coordinate>
 * @desc 金額テキストを表示するゾーン上の位置。右寄せ(Xが大きいほど左に表示される)。
 * @default {"x":"32","y":"48"}
 * 
 * @param showCards
 * @text カードを見せる？
 ...
 * @//option Spread
 * @default No
 * @desc このゾーンにカードを表示する方法。
 * 
 * @param description
 * @text ハイライト説明
 * @type note
 * @desc 戦闘中にこのゾーンがハイライトされたときにヘルプウィンドウに表示される内容。
 * 
 * @param enterSFX
 * @text カード入力SFX
 * @type file
 ...
 * @dir audio/se/
 * @require 1
 * @desc カードがこのゾーンから出たときに再生されるSE
 * 
 * @param emptyActions
 * @textカードアクション(空)
 * @type note
 * @descこのゾーンから最後のカードが取り除かれたときにトリガーされるカードアクションの一覧。
 * 
 * @//param emptyActionExecution
 * @//text 空トリガーのカードアクション
 * @//type select
 ...
 * @//option After Moving + End of Action
 * @//default Before and After Moving Cards from This Zone
 * @//desc カードアクションオンエンプティがいつトリガーされるかを決定します。アクションの終了は、移動後。
 */
 ...
 * @text ゾーン座標
 * @type struct<Coordinate>
 * @desc 解決を待っている間、カードが画面上のどこに移動するか
 *
 * @param cardScale
 * @text カードスケール
 * @type number
 * @decimals 4
 * @descこのゾーンに移動すると、カードスケールが変化します。
 * @default 0.5
 * 
 * @param skew
 * @text 歪み
 ...
 * @text 中心座標
 * @type struct<Coordinate>
 * @desc 画面上の手札の中心がどこになるか。一般的には画面の下部中央が推奨されます。
 * @default {"x":"408","y":"554"}
 * 
 * @param selectedCardYOff
 * @text 選択したカードYオフセット
 * @type number
 * @default 40
 * @desc 現在選択されているカードが他の手札より上に上げるピクセル数
 *
 * @param discardingCardYOff
 * @text カードYオフセットを破棄
 * @type number
 * @default 40
 * @desc カードが破棄されると、ピクセル数が増加します。
 *
 * @param inactiveCardYOff
 * @text 非アクティブハンドYオフセット
 * @type number
 * @default 40
 * @desc 現在、プレイヤーがカードを選択できない場合、すべてのカードのピクセル数が下がります。
 *
 * @param maxHandWidth
 * @text 手札の最大幅
 * @type number
 * @default 816
 * @desc 手札が占めることのできる最大の水平方向の空間。
 *
 * @param minCardSeparationW
 * @text 最小カード分離W
 * @type number
 * @default 60
 * @desc カード間のピクセル単位の最小スペース。そうでなければ最大幅に達してしまう。
 *
 * @param cardRotationMulti
 * @text カードローテーション倍率
 * @type number
 ...
 * @min -100
 * @default 1.00
 * @desc カードが回転する量は、手札の中心からどれだけ離れているかによって決まります。
 *
 * @param cardHeightMulti
 * @textカード高さ倍率
 * @type number
 ...
 * @min -100
 * @default 1.00
 * @desc カードの高さは、手元の中心からの距離によって変化します。これにより、その値に倍率が加わります。
 *
 *
 * @param enterSFX
 * @text カード入力SFX
 ...
 * @text カードアクション(空)
 * @type note
 * @desc 手札から最後のカードが取り除かれたときに起こるカードアクションの一覧。
 * @default ""
 * 
 * @param emptyActionExecution
 * @text 空トリガーのカードアクション
 ...
 * @option On Empty + End of Action
 * @default On Empty + Draw
 * @desc 「カードアクションオンエンプティ」がいつトリガーするかを決定します。
「ドロー」とは、このゾーンが空の状態でカードアクションがカードを移動させようとすることを意味します。
 */
