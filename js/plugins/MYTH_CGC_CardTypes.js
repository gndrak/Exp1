 ...
 * @target MZ * 
 * @author Isiah Brighton
 * @plugindesc v1.2.2 カードにタイプを追加する、MYTH_CGC_CoreEngineの拡張プラグインです。
 * @url https://www.patreon.com/mythatelier
 * 
 * 
 * @param types
 ...
 *     @type boolean
 *     @default true
 *     @desc ONに設定すると、カード・\タイプの名前がそのタイプのすべてのカードに表示されます。Falseに設定すると、アイコンのみが表示されます。
 *     
 *     @param nameCoordinate
 *     @text 座標
 *     @parent Appearance
 *     @type struct<Coordinate>
 *     @desc カードに描かれる最初のカードタイプのX/Y座標。
 *     @default {"x":"0","y":"0"}
 *     
 *     @param maxCols
 *     @text 列数
 ...
 *     @type number
 *     @default 2
 *     @desc 次の行に移動する前に Card に表示される Types の列数。
 *     
 *     @param typeWidth
 *     @text 列幅
 *     @parent Appearance
 *     @type number
 *     @default 172
 *     @desc タイプの各行の幅。タイプ間の間隔は、この値 + 列数に基づいて計算されます。
 *     
 *     @param typeHeight
 *     @text 列の高さ
 *     @parent Appearance
 ...
 *    @type number
 *    @default 16
 *    @desc アイコンはカード上に縮小して描画する必要がある場合があります。これは描画するアイコンのピクセル幅/高さです。
 *
 *
 *
 * @help
 ...
 * ============================================================================
 * 
* このプラグインは、各スキルに1つ以上のカードタイプを追加することができます。
 * 
 *  まず、プラグインパラメータでタイプを定義します。タイプを定義する際、Name以外のパラメータはカードへの描画方法に影響します。
 * 
 * 次に、スキルのメモタグを使って、作成したカードタイプをスキルに与えます。
 * スペルや大文字小文字は全く同じでなければならないので、タイプミスに注意してください。 
* 
 * ============================================================================
 * スキルメモタグ
 * ============================================================================
 * 
* <Card Type: type_name>
 * “type_name "を、このスキルに持たせたいカードタイプ名（大文字と小文字を区別します）に置き換えてください。
 * スキル/カードは、このメモタグの複数のインスタンスを使用して、複数のカードタイプを持つことができます。
 * 
 * 使用するすべての Type が、Card Types プラグインのパラメータに含まれていることを確認してください。
 * 
 *  MYTH_CGC_CardTypesの<Card Base: file_name>メモタグを含むスキルは、このプラグインで指定されたCard Baseの代わりに、指定されたCard Baseを使用します。
 * ============================================================================
 * エクストラカードアクション
 * ============================================================================
 * 
  * Draw X Type typeName
 *     指定されたタイプのカードをデッキから最初にX枚引く。
 *     
 *     
 * Mill X Type typeName
 * Discard X Type typeName
 * Remove X Type typeName
 *     これらは同じように動作しますが、ミル/捨て札/除去のそれぞれのアクションに対応しています。
 *     
 * Mill Until Type typeName
 *     山札の一番上のカードが指定された種類のカードになるまで、山札の一番上からカードをミルする。
 *     
 * XタイプtypeNameをzoneNameからzoneNameに移動する。
 *これは全てのゾーンを受け付ける汎用Moveアクションです。
 *     
 * ============================================================================
 * 上級者向け機能
 * ============================================================================
 * 
  * user.cardsInZoneOfType(zone, type)
 *     この関数はダメージ計算式、Eval、Requireのメモタグの内部で使 うことができる。この関数は指定されたゾーンにある指定されたタイプのカードの量を返します。
 *     
 * 'zone'と'type'は文字列パラメータであり、引用符で囲む必要がある。     
 *     "hand"
 *     "deck"
 *     "discard"
 *     
 * 'type'はカードタイプ名で、引用符で囲む。
 * 
 * 書式の例:
 * 
 *     <Card Passives>
 ...
 * ============================================================================
 * 
 * v1.2.2 -  Type のカードへの描画方法を、Selectable Window でのアイテムの描画方法、
 *          つまりグリッドに合わせるように変更。
 *          列数、行幅、行高さのパラメータを追加しました。
 *          TextFormatPlus にある冗長なパラメータを削除しました。
 *          大文字と小文字を区別しません。
 *          Card Types が存在する場合、Library Scene の変更は継続されます。
 *          新しい関数 drawRandomCardsOfType(amount, type)を追加。楽しいかもしれません。
 *          カード/ゾーンのゲッター関数を増やしました。
 *	　　Myth.Util.tryParseをCardTypesからCoreEngineに移動。
 * 
* v1.2.1 - アイコンフォーマットの問題を修正し、新しいプラグインパラメータを追加しました。
 * 間違った ResetFontSettings が使用されていた TextFormatPlus のバグを修正。
 *
 * v1.2.0 - v1.6.0 との互換性のために更新。
 * プラグイン名を変更しました。
 * 他の拡張との互換性を向上させました。
 * 新しいカードアクションを追加しました。
 * zoneNameに移動します。
 *
 * v1.1.0 - Mill Until Type カードアクションのクラッシュを修正しました。
 *
 * v1.0.9 - 破棄を確認するためにターン終了ボタンが選択できないバグを修正。
 *      
 *          カードの種類Xを捨てる/取り除くアクションで、その種類のカードが足りないとソフトロックしてしまうバグを修正。
 * 
 * v1.0.8 - カードバトル無効時にスキルが表示されないバグを修正。
 *         
 * 
 * v1.0.7 - 型構造体にエラー処理を追加し、未定義の値が MZ や MV ゲームをクラッシュさせないように。代わりにデフォルト値が提供され、コンソールを通じて警告が表示されるようになった。
 *         
 * 
 * v1.0.6 - 型構造体の読み込みをクリーンアップ。
 *     [ライブラリに表示]がOFFに設定されているタイプは、カードライブラリに表示されません。
 *     “Show in Deck Editor "がOFFに設定されているタイプは、デッキに表示されません。
 *     MYTH_CGC_DeckEditorCoreを使用している場合、エディターに表示されません。
 * 
 * v1.0.5 - CGC v1.5.1との互換性のために更新されました。
 * 
 * v1.0.4 - プラグインパラメータで定義されていないTypeを持つカードを使用した場合のクラッシュを修正。Typeリストが空の場合のエラーを修正。
 *    
 * 
 * v1.0.3 - スキルのシーンを開いたときのクラッシュを修正。
 * 
 * v1.0.2 - MZカードライブラリのクラッシュを修正。
 * 
 * v1.0.1 - プラグインリリース。
 * 
 */
 ...
 * @type boolean
 * @default true
 * @desc ON に設定すると、このカードタイプがカードに描かれる。OFF の場合、以下のパラメータはすべて無視される。
 *
 * @param icon
 * @text アイコン
 * @type number
 * @desc カードに表示する場合は、カードタイプを表すアイコンインデックス。
 * @default 0
 * 
 * @param cardBase
 * @text カードベース
 * @type string
 * @desc このタイプのすべてのカードに使用するカードベース。この機能を無効にするには何も設定しない。
 * 
 * @param showLibrary
 * @text Sライブラリを見る？
 * @type boolean
 * @default true
 * @desc ONに設定すると、カードライブラリでこのタイプでフィルタリングできるようになります。
 * 
 * @param showDeck
 * @text デッキエディタを見る？
 * @type boolean
 * @default true
 * @desc ONに設定すると、デッキ統計ウィンドウにカテゴリーアイコンが表示される。
 * 
* @param showCollection
* @text コレクションを見る？
* @type boolean
* @default true
* @desc ON に設定すると、プレーヤーはコレクションでこのタイプでフィルタリングできるようになります。
 * 
 */
 ...
 * @text カードベース
 * @type string
 * @desc このタイプのすべてのカードに使用するカードベース。何も設定しないとこの機能は無効になる。
 * 
 * @param showLibrary
 * @text ライブラリを見る？
 * @type boolean
