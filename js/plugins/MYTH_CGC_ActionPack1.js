 ...
 * @target MZ *
 * @author Isiah Brighton
 * @plugindesc v1.1.1 MythCardGameCombatの拡張機能で、カードアクションが追加されました。
 * @url https://www.patreon.com/mythatelier
 *
 * @param selectionHighlight
 * @text 選択ハイライト画像
 ...
 * @dir img/system
 * @require 1
 * @desc カードがセレクションウィンドウで選択待ちになっているときに、カードの上に表示されるスプライト。
 *
 * @param showTopText
 * @text トップテキストを表示する？
 * @type boolean
 * @default true
 * @desc カード選択ウィンドウにトップテキストを表示するかどうかを決定する。トップテキストは "zoneNameからX枚選択 "です。
 * 
 * @param showBottomText
 * @text一番下のテキストを表示する？
 * @type boolean
 * @default true
 * @desc 選択中のカード選択ウインドウに下部のテキストが表示されるかどうかを決定します。下部のテキストは "X / X selected "です。
 * 
 * @help
 *
 * ============================================================================
 ...
 * ============================================================================
 *
 * このプラグインは、カード選択ウィンドウに関連するいくつかの新しいカードアクションを追加します。
 * 
 * これらのカードアクション(CA)は、3つのステージのうち1つのステージで、グループとして機能します：
 * stages:
 * 1. 選択
 * 2. 移動
 * 3. クリア
 * 
 * 
 * 選択 CA は選択ウィンドウを開き、プレイヤーにカードを選択させます。
 *     (例:「デッキから 2 枚選択」)
 * 
 * 移動 CA は、選択 CA で選択されたカードを含むアクションを実行する
 *      (例：「選択されたカードを捨て札に移動」)
 *     
 * クリアCAは、選択されたカードのリストをクリアし、将来のムーブメントCAがそれらを使用しないようにする
 *     （「選択されたカードのクリア」）。
 *     
 *  選択カードアクションは、移動カードアクションが実行される前に呼び出される
 * 必要があり、クリアカードアクションは、次のカードアクションの準備のためにす
 * べての内部変数をクリーンアップします。
 * 
 * 例
 *     <Card Actions>
 ...
 *     Clear selected
 *     </Card Actions>
 * この例では、プレイヤーは山札から好きなカード2枚を捨札に移す。
 * 
 * 
 *
 * ============================================================================
 ...
 *
 * Select X from zoneName
 *     'X'を数字かゲーム変数に置き換え、'zoneName'を'deck'、
 *     'discard'、'hand'に置き換える。
 *     これはカード選択ウィンドウを開き、プレイヤーに指定されたゾーンから
 *     X枚のカードを選ばせます。
 *     
 * Select X of Y from zoneName location
 *     ‘X’を数字に、’Y’を別の数字に、’zoneName’をゾーンに、
 *     ‘location’を’Top’ ’Bottom’ ‘Random’のいずれかに置き換える。
 *     プレイヤーは指定されたゾーンにあるY枚のカードの中からX枚を選ぶ。
 *     ‘Top’の場合、カードのプールは一番上から、’Bottom'の場合は一番下から、
 *     ' random'の場合、カードはゾーンからランダムに取り出される。
*     
 * Select X from skills Y, Z, A, B
 *     X、Y、Zなどを数字に置き換える。
 *     プレイヤーにスキルIDのリストからX枚のカードを選ばせる。
 *     IDのリストはカンマで区切られた任意のサイズでよい。
 * 
 * 
 * If you're using CardTypes:
 * 
 * 
 * Select X Type typeName from zoneName
 *     Xを数字に、typeNameをカード・タイプの名前に、
 *     zoneNameをゾーン に置き換えてください。
 *     プレイヤーは指定されたゾーンから、指定されたカード・タイ プのみを含むように
 *     フィルタリングされたカードをX枚選択する。
 *     
 * Select X of Y Type typeName from zoneName location
 *     指定されたゾーンから X 枚選び、指定されたカードタイプのみを含むように
 *     フィルターをかけ、プールのサイズを Y とし て、一番上/一番下から、
 *     またはゾーンから無作為に選びます。
 *     
 * Select All from zoneName
 *     指定したゾーンのすべてのカードが自動的に「選択済み」にロードされる。
 *     
 *     
 * 
 * 各選択CAは、選択されたカードのリストを上書きするのではなく、
 * 追加することに注意。
 *
 * ============================================================================
 * 移動カードのアクション
 * ============================================================================
 *
 * プレイヤーが選択を行った後、これらの移動カードアクションは
 * 選択されたカードを使って何かを行います。
 * 
 * Move selected to zoneName
 *     ‘zoneName'をゾーンに置き換えてください。
 *     選択されたカードを前のゾーンから指定されたゾーンに移動させる。
 *     
 *     
 * Remove selected
 *     選択されたカードはプレイから取り除かれる。
 *     
 * Add selected to zoneName
 *     ‘zoneName'をゾーンに置き換えてください。
 *     選択されたカードのコピーを指定されたゾーンに追加します。
 *     カードアクション「スキルから選択」を使用する際に機能する、
 *     唯一の 移動カードアクションであることに注意してください。
*     
 *
 * ============================================================================
 * クリアカードアクション
 * ============================================================================
 *各選択カードアクションは選択カードのリストに追加され、
 *そのリストは以下のいずれかをコールするまでクリアされない：
 * 
 * Clear Selected
 * Clear Selection
 * 
 * つまり、「選択／選択解除」をコールするまでは、複数の「選択CA」と、
 * そのすべてに作用する「移動CA」を1回ずつ実行したり、選択したカード1枚だけで
 * 複数の「移動CA」をコールしたりすることができます。
 * 
 *
 * ============================================================================
 * バージョン履歴
 * ============================================================================
 * 
 * v1.1.1 - カードプールの長さを0にするとクラッシュする問題を修正。
 *    
 * 
 * v1.1.0 - すべてのカードを選択するアクションを追加。
 *          カードを通常閲覧している間、選択テキストが表示されなくなりました。
 *          CGC core v1.6.0とCard Summonsとの互換性を更新しました。
 *          カードアクションの変数の読み込みを改善するためのマイナーなリファクタリング。
 *          プラグインの名前を変更。
 *          カード選択ウィンドウにテキストを表示しないオプションを追加。
 *          カード選択ウィンドウにテキストを表示しないオプションを追加。
 *          
 *
 * v1.0.0 - プラグインリリース
